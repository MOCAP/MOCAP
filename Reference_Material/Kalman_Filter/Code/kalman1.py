...
class Voltmeter:
  ...
  def GetVoltage(self,time):
	if  time < 1000000:
		return self.truevoltage + time
	else :
		return self.truevoltage - time +60
  def GetVoltageWithNoise(self,time):
    return random.gauss(self.GetVoltage(time),self.noiselevel)
...
for i in range(numsteps):
    measured = voltmeter.GetVoltageWithNoise(i)
    measuredvoltage.append(measured)
    truevoltage.append(voltmeter.GetVoltage(i))
